...
- [ ] Bootstrap and protect the remote Terraform state store: use a private S3 bucket with encryption, versioning, and public-access blocking; restrict current and historical object versions with least-privilege IAM; enable state locking; prevent downloaded state files from being committed; and treat state access as secret access because the state can contain the Redis token even when Terraform redacts it from plan output.
