...
if (BigInt(latestBlock.timestamp) < poolInfo.maturityTime) {
...
if ((await pool.getPoolState(poolId)) !== REPAID) {
  throw new Error(`expected pool ${poolId} to enter the REPAID state`);
}
console.log(`Repaid on-chain pool ${poolId} in block ${receipt.blockNumber}`);
await waitForIndexedState(poolId, deployment.chainId, REPAID.toString());
console.log(`Backend indexed pool ${poolId} state REPAID`);

async function validatePreparedProposal(
  preparedProposal: PreparedProposal,
  expectedNonce: string,
) {
  if (preparedProposal.proposal.operation !== "repay_pool") {
    throw new Error(
      `API returned operation ${preparedProposal.proposal.operation}, expected repay_pool`,
    );
  }
  if (
  ...
  if (BigInt(preparedProposal.proposal.value) !== 0n) {
    throw new Error("repay proposal must not transfer native currency");
  }
  ...
  if (decoded[0] !== poolId || decoded[1] !== maxCollateralAmount) {
    throw new Error(
      `API encoded repayPool(${decoded[0]}, ${decoded[1]}), expected repayPool(${poolId}, ${maxCollateralAmount})`,
    );
  }
